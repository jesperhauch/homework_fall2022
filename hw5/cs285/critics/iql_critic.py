...
class IQLCritic(BaseCritic):
    ...
    def expectile_loss(self, diff):
        ...
        L_2 = torch.pow(diff, 2) * torch.abs(self.iql_expectile - indicator)
        return torch.mean(L_2)
    ...
    def update_v(self, ob_no, ac_na):
        ...
        ### YOUR CODE HERE ###
        q_vals = torch.gather(self.q_net_target(ob_no), 1, ac_na.unsqueeze(1))
        value_loss = self.expectile_loss(q_vals.detach() - self.v_net(ob_no))
    ...
    def update_q(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        targets = reward_n.unsqueeze(1) + self.gamma*self.v_net(next_ob_no)
